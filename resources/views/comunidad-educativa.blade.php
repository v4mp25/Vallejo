...
<head>
    ...
    <title>Comunidad Educativa - I.E. César Vallejo</title>
    ...
    <style>
        ...
        .cv-ally-card { transition: transform 0.25s ease, box-shadow 0.25s ease; border: 1px solid #eee !important; border-radius: 12px; overflow: hidden; background: #fff; }
        .cv-ally-card:hover { transform: translateY(-5px); box-shadow: 0 10px 20px rgba(1,72,164,0.1) !important; }
    </style>
</head>
<body>
        ...
        <nav class="navbar navbar-expand-xl navbar-dark px-4 px-lg-5 py-2">
            ...
            <div class="collapse navbar-collapse" id="navbarCollapse">
                <div class="navbar-nav ms-auto align-items-lg-center gap-lg-1">
                    ...
                    <div class="nav-item dropdown cv-dropdown-hover">
                        <a class="nav-link dropdown-toggle text-white fw-bold" href="{{ url('/comunidad-educativa') }}" id="navbarComunidad" style="color: #ffc107 !important;">
                            Comunidad
                        </a>
    ...
    <header class="header-institucion">
        <div class="container">
            <h1 class="fw-bold mb-3"><i class="fas fa-users me-2"></i> Comunidad Educativa</h1>
            <p class="lead opacity-75">Los pilares que sostienen, dan vida y nos vinculan a la sociedad.</p>
        </div>
    </header>
        ...
        <div class="row g-4">
            ...
            <div class="col-lg-3">
                <div class="nav flex-column nav-pills nav-pills-custom" id="v-pills-tab" role="tablist" aria-orientation="vertical">
                    <button class="nav-link active" data-bs-toggle="pill" data-bs-target="#estudiantes" type="button">
                        <i class="fas fa-user-graduate me-2"></i> 6.1 Estudiantes
                    </button>
                    <button class="nav-link" data-bs-toggle="pill" data-bs-target="#padres" type="button">
                        <i class="fas fa-user-friends me-2"></i> 6.2 Padres de Familia
                    </button>
                    <button class="nav-link" data-bs-toggle="pill" data-bs-target="#exalumnos" type="button">
                        <i class="fas fa-user-tie me-2"></i> 6.3 Exalumnos
                    </button>
                    <button class="nav-link" data-bs-toggle="pill" data-bs-target="#aliados" type="button">
                        <i class="fas fa-handshake me-2"></i> 6.4 Aliados Estratégicos
                    </button>
                </div>
            </div>
            ...
            <div class="col-lg-9">
                <div class="tab-content tab-content-box" id="v-pills-tabContent">
                    ...
                    <div class="tab-pane fade show active" id="estudiantes" role="tabpanel">
                        ...
                        <p class="text-muted lh-lg" style="white-space: pre-line;">{{ $info->estudiantes_texto ?? 'Nuestros estudiantes son el centro del proceso de aprendizaje. Promovemos el liderazgo, el respeto y la innovación para potenciar su crecimiento integral.' }}</p>
                        
                        @if($info->reglamento_pdf)
                            <div class="mt-5 p-4 bg-light rounded-4 border d-flex flex-column flex-sm-row justify-content-between align-items-sm-center gap-3">
                                <div>
                                    <h5 class="fw-bold text-dark mb-1"><i class="fas fa-file-pdf text-danger me-2"></i> Reglamento Escolar</h5>
                                    <p class="text-muted small mb-0">Revisa los lineamientos de convivencia, derechos y deberes del estudiante.</p>
                                </div>
                                <a href="{{ asset('storage/' . $info->reglamento_pdf) }}" target="_blank" class="btn btn-primary rounded-pill px-4 fw-bold shadow-sm align-self-start align-self-sm-center">
                                    <i class="fas fa-download me-1"></i> Descargar Reglamento
                                </a>
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="tab-pane fade" id="padres" role="tabpanel">
                        <h3 class="fw-bold text-dark border-bottom pb-2 mb-4" style="border-bottom-color: var(--cv-light-bg) !important;">
                            <i class="fas fa-user-friends me-2 text-success"></i> Padres de Familia
                        </h3>
                        <p class="text-muted lh-lg" style="white-space: pre-line;">{{ $info->padres_texto ?? 'La alianza entre la escuela y el hogar es primordial. Trabajamos mano a mano con la APAFA y los comités de aula para velar por el bienestar integral de los alumnos.' }}</p>
                        
                        @if($info->cronograma_notes_pdf)
                            <div class="mt-5 p-4 bg-light rounded-4 border d-flex flex-column flex-sm-row justify-content-between align-items-sm-center gap-3">
                                <div>
                                    <h5 class="fw-bold text-dark mb-1"><i class="fas fa-calendar-alt text-primary me-2"></i> Cronograma de Evaluaciones</h5>
                                    <p class="text-muted small mb-0">Descarga el cronograma de notas y exámenes para realizar el seguimiento académico.</p>
                                </div>
                                <a href="{{ asset('storage/' . $info->cronograma_notes_pdf) }}" target="_blank" class="btn btn-success rounded-pill px-4 fw-bold text-white shadow-sm align-self-start align-self-sm-center">
                                    <i class="fas fa-download me-1"></i> Descargar Cronograma
                                </a>
                            </div>
                        @endif
                    </div>
                    ...
                    {{-- 6.3 Exalumnos --}}
                    <div class="tab-pane fade" id="exalumnos" role="tabpanel">
                        <h3 class="fw-bold text-dark border-bottom pb-2 mb-4" style="border-bottom-color: var(--cv-light-bg) !important;">
                            <i class="fas fa-user-tie me-2 text-warning"></i> Exalumnos / Egresados
                        </h3>
                        <p class="text-muted lh-lg" style="white-space: pre-line;">{{ $info->exalumnos_texto ?? 'Nuestros egresados continúan portando con orgullo la identidad vallejiana. Fomentamos redes de egresados para compartir experiencias profesionales y mentorías.' }}</p>
                    </div>

                    {{-- 6.4 Aliados Estratégicos --}}
                    <div class="tab-pane fade" id="aliados" role="tabpanel">
                        <h3 class="fw-bold text-dark border-bottom pb-2 mb-4" style="border-bottom-color: var(--cv-light-bg) !important;">
                            <i class="fas fa-handshake me-2 text-info"></i> Aliados Estratégicos
                        </h3>
                            ...
                            <div class="row g-4 mt-2">
                                @foreach($aliados as $aliado)
                                    <div class="col-md-6 col-xl-4">
                                        <div class="cv-ally-card h-100 p-4 d-flex flex-column justify-content-between text-center">
                                            <div>
                                                <div class="d-flex align-items-center justify-content-center bg-light rounded-3 mb-3" style="height: 90px;">
                                                    <img src="{{ asset('storage/' . $aliado->logo) }}" class="img-fluid" style="max-height: 70px; object-fit: contain;" alt="Logo {{ $aliado->nombre }}">
                                                </div>
                                                <h6 class="fw-bold text-dark mb-2">{{ $aliado->nombre }}</h6>
                                                <p class="text-muted small mb-4">{{ $aliado->descripcion }}</p>
                                            </div>
                                            
                                            @if($aliado->enlace_web)
                                                <div>
                                                    <a href="{{ $aliado->enlace_web }}" target="_blank" class="btn btn-outline-primary btn-sm rounded-pill px-4 fw-bold">
                        ...
                        @else
                            <div class="text-center py-5 text-muted">
                                <i class="fas fa-handshake fs-1 mb-3 text-warning opacity-50"></i>
                                <p class="mb-0 fw-semibold">No se tienen aliados estratégicos registrados aún.</p>
                                <small class="text-muted d-block mt-1">Estamos gestionando nuevos convenios con universidades e institutos de prestigio.</small>
                            </div>
                        @endif
                    </div>
