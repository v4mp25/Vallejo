...
<body>
    <nav class="navbar navbar-expand-lg bg-white border-bottom shadow-xs">
        <div class="container">
            ...
            <div class="d-flex align-items-center gap-2">
                <button type="button" class="btn btn-primary btn-sm btn-action px-3" data-bs-toggle="modal" data-bs-target="#modalPublicarMaterial">
                    <i class="fas fa-plus-circle me-1"></i>Publicar Nuevo Material/Test
                </button>
    ...
    <main class="container py-4">
        ...
        <div class="card border-0 shadow-sm rounded-4 mb-4 text-white overflow-hidden" style="background: linear-gradient(135deg, #0d6efd 0%, #0a58ca 100%);">
            <div class="card-body p-4 position-relative" style="z-index: 1;">
                <div class="row align-items-center">
                    ...
                    <div class="col">
                        ...
                        <h3 class="fw-bold mb-0">Psicólogo(a): {{ Auth::user()->nombres }} {{ Auth::user()->apellidos }}</h3>
            ...
            <div class="col-md-6">
                <div class="card card-stats border-0 shadow-sm rounded-4 border-start border-4 border-warning h-100">
                    <div class="card-body p-4 d-flex align-items-center justify-content-between">
                        <div>
                            <small class="text-muted d-block fw-semibold text-uppercase" style="font-size: 0.75rem; letter-spacing: 0.5px;">Citas / Derivaciones Activas</small>
        ...
        <div class="card border-0 shadow-sm rounded-4">
            <div class="card-header bg-white border-bottom-0 pt-4 pb-2 px-4">
                <h5 class="mb-0 fw-bold text-dark"><i class="fas fa-clipboard-list text-primary me-2"></i>Derivaciones y Citas Pendientes</h5>
            </div>
                            ...
                            <tbody>
                                @foreach ($pendientes as $cita)
                                    ...
                                    <tr>
                                        ...
                                        <td><span class="badge bg-light text-dark border px-2.5 py-1.5 rounded-pill">{{ $gradoSeccion }}</span></td>
                                        <td><span class="badge bg-secondary-subtle text-secondary px-2.5 py-1.5 rounded-pill fw-semibold">{{ $labelBimestre }}</span></td>
                                        ...
                                        <td>
                                            @if($cita->fecha_cita)
                                                <span class="fw-semibold text-primary"><i class="far fa-calendar-alt me-1"></i>{{ \Carbon\Carbon::parse($cita->fecha_cita)->format('d/m/Y h:i A') }}</span>
                                            @else
                                                <span class="text-muted italic"><i class="far fa-calendar-minus me-1"></i>Sin programar</span>
                                            @endif
                                        </td>
                                        <td>
                                            @if($cita->estado === 'citado')
                                                <span class="badge bg-info text-white rounded-pill px-3 py-1.5 fw-semibold">Citado</span>
                                            @else
                                                <span class="badge bg-warning text-dark rounded-pill px-3 py-1.5 fw-semibold">Pendiente</span>
                                            @endif
                                        </td>
                                        <td class="text-end pe-3">
                                            <div class="d-flex align-items-center justify-content-end gap-1">
                                                <button
                                                    type="button"
                                                    class="btn btn-sm btn-action {{ $cita->fecha_cita ? 'btn-outline-primary' : 'btn-primary' }} px-3"
                                                ...
                                                @if($cita->fecha_cita)
                                                    <form method="POST" action="{{ route('psicologo.citas.atendida', $cita) }}" class="d-inline" onsubmit="return confirm('¿Marcar al alumno como atendido en su cita?')">
                                                        @csrf
                                            ...
                                            <div class="modal-content border-0 shadow-lg rounded-4">
                                                <form method="POST" action="{{ route('psicologo.citas.asignar', $cita) }}">
                                                    ...
                                                    <div class="modal-footer bg-light border-top-0 pt-0">
                                                        ...
                                                        <button type="submit" class="btn btn-primary rounded-pill px-4 fw-bold">Guardar Cita</button>
                                                    </div>
                                                </form>
                                            </div>
                                        </div>
                    ...
                    <div class="accordion rounded-4 overflow-hidden border" id="accordionHistorial">
                        @foreach ($atendidasPorBimestre as $bim => $citasAtendidas)
                            ...
                            <div class="accordion-item border-0 border-bottom">
                                <h2 class="accordion-header" id="heading{{ $bim }}">
                                    <button class="accordion-button collapsed fw-bold text-dark bg-light" type="button" data-bs-toggle="collapse" data-bs-target="#collapse{{ $bim }}" aria-expanded="false" aria-controls="collapse{{ $bim }}">
                                        <i class="fas fa-folder text-primary me-2"></i> {{ $label }} 
                                        <span class="badge bg-primary rounded-pill ms-2 px-2.5 py-1 text-xs fw-semibold">{{ $citasAtendidas->count() }} @choice('atención|atenciones', $citasAtendidas->count())</span>
                                    </button>
                                </h2>
                                <div id="collapse{{ $bim }}" class="accordion-collapse collapse" aria-labelledby="heading{{ $bim }}" data-bs-parent="#accordionHistorial">
        ...
        {{-- HISTORIAL DE MATERIALES PUBLICADOS POR EL PSICÓLOGO --}}
        <div class="card border-0 shadow-sm rounded-4 mt-4">
            <div class="card-header bg-white border-bottom-0 pt-4 pb-2 px-4">
                <h5 class="fw-bold text-dark mb-0">
                    <i class="fas fa-folder-open text-primary me-2"></i> Mis Materiales y Tests Publicados
                </h5>
            </div>
            <div class="card-body px-4 pb-4">
                @if ($materialesPorBimestreDoc->isEmpty())
                    <p class="text-muted mb-0 py-4 text-center fst-italic">No has publicado ningún material o evaluación aún.</p>
                @else
                    <div class="accordion rounded-4 overflow-hidden border" id="accordionMaterialesDoc">
                        @foreach ($materialesPorBimestreDoc as $bim => $materialesDoc)
                            <div class="accordion-item border-0 border-bottom">
                                <h2 class="accordion-header" id="headingDoc{{ Str::slug($bim) }}">
                                    <button class="accordion-button collapsed fw-bold text-dark bg-light" type="button" data-bs-toggle="collapse" data-bs-target="#collapseDoc{{ Str::slug($bim) }}" aria-expanded="false" aria-controls="collapseDoc{{ Str::slug($bim) }}">
                                        <i class="fas fa-bookmark text-primary me-2"></i> {{ $bim }} 
                                        <span class="badge bg-primary rounded-pill ms-2 px-2.5 py-1 text-xs fw-semibold">{{ $materialesDoc->count() }} recurso(s)</span>
                                    </button>
                                </h2>
                                <div id="collapseDoc{{ Str::slug($bim) }}" class="accordion-collapse collapse" aria-labelledby="headingDoc{{ Str::slug($bim) }}" data-bs-parent="#accordionMaterialesDoc">
                                    <div class="accordion-body p-0">
                                        <div class="table-responsive">
                                            <table class="table align-middle mb-0 table-hover">
                                                <thead class="table-light">
                                                ...
                                                <tbody>
                                                    @foreach ($materialesDoc as $mat)
                                                        <tr>
                                                            ...
                                                            <td>
                                                                @if($mat->enlace)
                                                                    <a href="{{ $mat->enlace }}" target="_blank" class="badge bg-light text-primary border text-decoration-none"><i class="fas fa-link me-1"></i>Link</a>
                                                                @else
                                                            ...
                                                            <td class="text-end pe-4">
                                                                <form action="{{ route('psicologo.materiales.eliminar', $mat->id) }}" method="POST" onsubmit="return confirm('¿Estás seguro de eliminar este recurso?')">
                                                                    @csrf
                                                                    @method('DELETE')
    ...
    {{-- MODAL PUBLICAR NUEVO MATERIAL/TEST --}}
    <div class="modal fade" id="modalPublicarMaterial" tabindex="-1" aria-labelledby="modalPublicarMaterialLabel" aria-hidden="true" data-bs-backdrop="static">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content border-0 shadow-lg rounded-4">
                <form method="POST" action="{{ route('psicologo.materiales.store') }}" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-header bg-primary text-white border-bottom-0 pb-0">
                        <h5 class="modal-title fw-bold" id="modalPublicarMaterialLabel">
                            <i class="fas fa-paper-plane me-2"></i>Publicar Nuevo Material / Test
                        </h5>
                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Cerrar"></button>
                    </div>
                    <div class="modal-body py-4">
                        <div class="mb-3">
                            <label class="form-label fw-bold text-secondary small">Título del Material o Test</label>
                            <input type="text" name="titulo" class="form-control rounded-3" placeholder="Ej: Test de Orientación Vocacional, Ficha de Autoestima" required value="{{ old('titulo') }}">
                        </div>
                        <div class="mb-3">
                            <label class="form-label fw-bold text-secondary small">Descripción (Opcional)</label>
                            <textarea name="descripcion" class="form-control rounded-3" rows="3" placeholder="Describe brevemente las instrucciones o el propósito de este recurso...">{{ old('descripcion') }}</textarea>
                        </div>
                        <div class="mb-3">
                            <label class="form-label fw-bold text-secondary small">Enlace / Link (Google Forms, Drive, etc.) (Opcional)</label>
                            <input type="url" name="enlace" class="form-control rounded-3" placeholder="Ej: https://docs.google.com/forms/d/..." value="{{ old('enlace') }}">
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            <div class="d-flex justify-content-between align-items-center mb-2">
                                ...
                                <button type="button" id="btn-select-all-aulas" class="btn btn-outline-secondary btn-sm rounded-pill px-3 py-1" style="font-size: 0.75rem; font-weight: 600;">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <div class="d-flex flex-wrap gap-2">
                                            @foreach($aulasGrupo as $aula)
                                                <input type="checkbox" class="btn-check check-aula-pub" name="aulas[]" value="{{ $aula->id }}" id="aula_pub_{{ $aula->id }}">
                                                <label class="btn btn-outline-primary btn-sm rounded-pill px-3 transition-all" for="aula_pub_{{ $aula->id }}">
                    ...
                    <div class="modal-footer bg-light border-top-0 pt-0">
                        <button type="button" class="btn btn-secondary rounded-pill px-4 fw-bold" data-bs-dismiss="modal">Cancelar</button>
                        <button type="submit" class="btn btn-primary rounded-pill px-4 fw-bold">Publicar Material</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // Seleccionar/Deseleccionar todas las aulas (toggle)
        const btnSelectAll = document.getElementById('btn-select-all-aulas');
        if (btnSelectAll) {
            let allChecked = false;
            btnSelectAll.addEventListener('click', function() {
                allChecked = !allChecked;
                document.querySelectorAll('.check-aula-pub').forEach(checkbox => {
