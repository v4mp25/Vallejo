...
<head>
    ...
    <title>Logros y Reconocimientos - I.E. César Vallejo</title>
    ...
    <style>
        ...
        .cv-logro-card { transition: transform 0.25s ease, box-shadow 0.25s ease; cursor: pointer; border: 1px solid #f1f1f1 !important; }
        .cv-logro-card:hover { transform: scale(1.02); box-shadow: 0 10px 25px rgba(1,72,164,0.15) !important; }
    </style>
</head>
<body>
    ...
    <header class="header-institucion">
        <div class="container">
            <h1 class="fw-bold mb-3"><i class="fas fa-trophy me-2"></i> Logros y Reconocimientos</h1>
            <p class="lead opacity-75">Nuestros hitos, medallas y reconocimientos en las diversas disciplinas.</p>
        </div>
    </header>
        ...
        <div class="row g-4">
            ...
            <div class="col-lg-3">
                <div class="nav flex-column nav-pills nav-pills-custom" id="v-pills-tab" role="tablist" aria-orientation="vertical">
                    <button class="nav-link active" data-bs-toggle="pill" data-bs-target="#academico" type="button">
                        <i class="fas fa-book-reader me-2"></i> 7.1 Académicos
                    </button>
                    <button class="nav-link" data-bs-toggle="pill" data-bs-target="#deportivo" type="button">
                        <i class="fas fa-running me-2"></i> 7.2 Deportivos
                    </button>
                    <button class="nav-link" data-bs-toggle="pill" data-bs-target="#artistico" type="button">
                        <i class="fas fa-palette me-2"></i> 7.3 Artísticos
                    </button>
                    <button class="nav-link" data-bs-toggle="pill" data-bs-target="#cientifico" type="button">
                        <i class="fas fa-flask me-2"></i> 7.4 Científicos
                    </button>
                    <button class="nav-link" data-bs-toggle="pill" data-bs-target="#institucional" type="button">
                        <i class="fas fa-award me-2"></i> 7.5 Reconocimientos
                    </button>
                </div>
            </div>
            ...
            <div class="col-lg-9">
                <div class="tab-content tab-content-box" id="v-pills-tabContent">
                    ...
                    @foreach($categorias as $key => $catName)
                        <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="{{ $key }}" role="tabpanel">
                            <h3 class="fw-bold text-dark border-bottom pb-2 mb-4" style="border-bottom-color: var(--cv-light-bg) !important;">
                                <i class="fas fa-award me-2 text-primary"></i> Logros {{ $catName }}
                            </h3>

                            @if(isset($logros[$catName]) && count($logros[$catName]) > 0)
                                <div class="row g-4">
                                    @foreach($logros[$catName] as $l)
                                        <div class="col-md-6">
                                            <div class="card border-0 shadow-sm rounded-4 overflow-hidden h-100 cv-logro-card" data-bs-toggle="modal" data-bs-target="#modalLogro{{ $l->id }}">
                                                <img src="{{ asset('storage/' . $l->imagen) }}" class="img-fluid" style="height: 180px; width: 100%; object-fit: cover;" alt="{{ $l->titulo }}">
                                                <div class="card-body p-4">
                                                    @if($l->fecha)
                                                        <span class="badge bg-light text-secondary border px-2 py-1 rounded-pill mb-2 small fw-semibold">
                                                            <i class="far fa-calendar-alt me-1"></i>{{ $l->fecha->format('d/m/Y') }}
                                                        </span>
                                                    @endif
                                                    <h5 class="fw-bold text-dark mb-2">{{ $l->titulo }}</h5>
                                                    <p class="text-muted mb-0 small lh-lg text-truncate">{{ $l->descripcion }}</p>
                                                </div>
                                            </div>
                                        </div>
                                    @endforeach
                            ...
                            @else
                                <div class="text-center py-5 text-muted">
                                    <i class="fas fa-award fs-1 mb-3 text-warning opacity-50"></i>
                                    <p class="mb-0 fw-semibold">No se tienen registros en la categoría {{ $catName }} aún.</p>
                                    <small class="text-muted d-block mt-1">El personal escolar está catalogando y digitalizando las actas de premiación.</small>
                                </div>
                            @endif
                        </div>
                    @endforeach
    ...
    {{-- Modales Lightbox --}}
    @foreach($logros as $cat => $items)
        @foreach($items as $l)
            <div class="modal fade" id="modalLogro{{ $l->id }}" tabindex="-1" aria-labelledby="modalLogroLabel{{ $l->id }}" aria-hidden="true">
                    ...
                    <div class="modal-content rounded-4 border-0 shadow">
                        ...
                        <div class="modal-body px-4 pb-4 pt-0">
                            <div class="text-center mb-4">
                                <img src="{{ asset('storage/' . $l->imagen) }}" class="img-fluid rounded-3 shadow-xs" style="max-height: 480px;" alt="{{ $l->titulo }}">
                            </div>
                            @if($l->fecha)
                                <span class="badge bg-light text-secondary border px-2 py-1 rounded-pill mb-2 small fw-semibold">
                                    <i class="far fa-calendar-alt me-1"></i>{{ $l->fecha->format('d/m/Y') }}
                            ...
                            @endif
                            <h4 class="fw-bold text-dark mb-3" id="modalLogroLabel{{ $l->id }}">{{ $l->titulo }}</h4>
                            <p class="text-muted lh-lg" style="white-space: pre-line;">{{ $l->descripcion }}</p>
                        </div>
